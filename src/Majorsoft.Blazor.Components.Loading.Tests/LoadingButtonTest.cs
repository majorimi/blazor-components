...
public class LoadingButtonTest : ComponentsTestBase<LoadingButton>
{
	...
	public void LoadingButton_should_rendered_correctly_html_attributes()
	{
		...
		button.MarkupMatches(
			"<button blazor:onclick=\"1\" type=\"button\" id=\"id1\" class=\"btn\">hello...</button>"
		);
	}
	...
	{
		var rendered = _testContext.RenderComponent<LoadingButton>(parameters =>
			parameters
	...
	public void LoadingButton_should_rendered_correctly_loading_state() //Incomplete test for technical reasons...
	{
		...
		rendered = _testContext.RenderComponent<LoadingButton>(parameters =>
			parameters
				.Add(p => p.Type, ButtonTypes.Submit)
				.Add(
					p => p.Content,
			...
			rendered.WaitForAssertion(
				() =>
					rendered.MarkupMatches(
						"<button blazor:onclick=\"1\" type=\"submit\" disabled=\"\">loading...</button>"
					),
	...
	public void LoadingButton_should_rendered_correctly_loading_state_enabled()
	{
		...
		rendered.WaitForAssertion(
			() =>
				rendered.MarkupMatches(
					"<button blazor:onclick=\"1\" type=\"submit\">loading...</button>"
				),
			timeout: TimeSpan.FromSeconds(1)
		);
	}
	...
	public void LoadingButton_should_be_default_state_when_IsLoading_false()
	{
		...
		rendered.WaitForAssertion(
			() =>
				rendered.MarkupMatches(
					"<button blazor:onclick=\"1\" type=\"submit\">hello...</button>"
				),
			timeout: TimeSpan.FromSeconds(2)
		);
	}
