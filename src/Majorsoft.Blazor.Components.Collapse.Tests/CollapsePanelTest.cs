...
public class CollapsePanelTest : ComponentsTestBase<CollapsePanel>
{
	...
	public void CollapsePanel_should_rendered_correctly_html_attributes()
	{
		...
		div.MarkupMatches(
			@$"<div class=""collapsePanel"" tabindex=""200"" title=""text""  >
	...
	{
		var rendered = _testContext.RenderComponent<CollapsePanel>(parameters =>
		...
		rendered.MarkupMatches(
			@$"<div class=""collapsePanel"" tabindex=""200"" disabled=""""  >
			  <div id=""{id}"" class=""collapseHeader animate"" style=""background-color: rgb(211,211,211);""    ></div>
			  <div class=""collapseContent animate"" style=""opacity: 1; overflow: hidden; max-height: 200px;"" ></div>
			</div>"
		);
	...
	public void CollapsePanel_should_rendered_correctly_CommonHeader()
	{
		...
		var id = div.GetAttribute("id");
		rendered.MarkupMatches(
			@$"<div class=""collapsePanel"" tabindex=""200"" >
	...
	{
		var rendered = _testContext.RenderComponent<CollapsePanel>(parameters =>
			parameters.Add(p => p.ExpandedHeaderContent, "Expanded header")
		);
	...
	{
		var rendered = _testContext.RenderComponent<CollapsePanel>(parameters =>
			parameters.Add(p => p.CollapsedHeaderContent, "Collapsed header")
		);
	...
	public void CollapsePanel_should_rendered_correctly_Content()
	{
		...
		rendered.SetParametersAndRender(parameters => parameters.Add(p => p.Collapsed, true));
		rendered.MarkupMatches(
			@$"<div class=""collapsePanel"" tabindex=""200"" >
			  <div id=""{id}"" class=""collapseHeader animate"" style=""background-color: rgb(211,211,211);"">Collapsed header</div>
	...
	{
		var rendered = _testContext.RenderComponent<CollapsePanel>(parameters =>
			parameters.Add(p => p.ContentHeight, 55)
		);
	...
	{
		var rendered = _testContext.RenderComponent<CollapsePanel>(parameters =>
			parameters.Add(p => p.Animate, false).Add(p => p.ShowContentOverflow, true)
		);
