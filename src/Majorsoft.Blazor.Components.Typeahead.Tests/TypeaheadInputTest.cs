...
public class TypeaheadInputTest : ComponentsTestBase<TypeaheadInput<string>>
{
	...
	[ClassInitialize]
	public static void TestInitForAll(
	...
	public void Init()
	{
		var logger = Substitute.For<ILogger<DebounceInput>>();
		var logger2 = Substitute.For<ILogger<AdvancedTimer>>();
		_clickBoundariesMock = Substitute.For<IClickBoundariesHandler>();

		_testContext.Services.Add(new ServiceDescriptor(typeof(ILogger<DebounceInput>), logger));
	...
	public void TypeaheadInput_should_rendered_correctly_html_attributes()
	{
		...
		input.MarkupMatches(
			@"<input autocomplete=""off"" id=""id1"" class=""form-control w-100"" />"
		);
	}
	...
	{
		var rendered = _testContext.RenderComponent<TypeaheadInput<string>>(parameters =>
			parameters
	...
	public async Task TypeaheadInput_should_rendered_correctly_DropdownHeight()
	{
		...
		rendered.WaitForAssertion(
			() =>
				input.MarkupMatches(
					@"<input value=""t"" autocomplete=""off"" class=""typeahead"" />"
				)
		);
	}
