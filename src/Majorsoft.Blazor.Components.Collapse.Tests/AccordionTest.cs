...
namespace Majorsoft.Blazor.Components.Collapse.Tests
{
	...
	public class AccordionTest : ComponentsTestBase<Accordion>
	{
		...
		{
			var rendered = _testContext.Render<Accordion>(parameters => parameters
		...
		{
			var rendered = _testContext.Render<Accordion>();

			var c1 = _testContext.Render<CollapsePanel>(parameters => parameters
					.Add(p => p.Parent, rendered.Instance)
					.Add(p => p.CommonHeader, "Header 1")
					.Add(p => p.Content, "content 1"));
			var c2 = _testContext.Render<CollapsePanel>(parameters => parameters
					.Add(p => p.Parent, rendered.Instance)
