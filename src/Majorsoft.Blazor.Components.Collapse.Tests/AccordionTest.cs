...
public class AccordionTest : ComponentsTestBase<Accordion>
{
	...
	public void Accordion_should_rendered_correctly_html_attributes()
	{
		...
		div.MarkupMatches(
			@$"<div class=""accordionPanel"" tabindex=""200"" title=""text""  ></div>"
		);
	}
	...
	public void Accordion_should_rendered_correctly_CollapsePanel()
	{
		...
		var c1 = _testContext.RenderComponent<CollapsePanel>(parameters =>
			parameters
				.Add(p => p.Parent, rendered.Instance)
				.Add(p => p.CommonHeader, "Header 1")
				.Add(p => p.Content, "content 1")
		);
		var c2 = _testContext.RenderComponent<CollapsePanel>(parameters =>
			parameters
				.Add(p => p.Parent, rendered.Instance)
	...
	public void Accordion_should_rendered_correctly_only_one_active_CollapsePanel()
	{
		...
		rendered.SetParametersAndRender(parameters =>
			parameters.Add(p => p.ActiveCollapsePanel, c2.Instance)
		);
		...
		rendered.SetParametersAndRender(parameters =>
			parameters.Add(p => p.ActiveCollapsePanel, c1.Instance)
		);
