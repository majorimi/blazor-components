...
{
	public abstract class TypeaheadBase<TItem> : ComponentBase, IAsyncDisposable
	{
		private readonly IClickBoundariesHandler _clickHandler;

		protected abstract ILogger BaseLogger { get; }
		protected abstract IClickBoundariesHandler ClickBoundariesHandler { get; }
		...
		private async Task OnFocus(FocusEventArgs e)
		{
			...
			await ClickBoundariesHandler.RegisterClickBoundariesAsync(_typeahead.InnerElementReference, OnOutsideClick, OnInsideClick);
		...
		private async Task OnOutsideClick(MouseEventArgs e)
		{
			...
			await ClickBoundariesHandler.RemoveClickBoundariesAsync(_typeahead.InnerElementReference);
		...
		{
			if (ClickBoundariesHandler is not null)
			{
				await ClickBoundariesHandler.DisposeAsync();
			}
		}
	}
}